...
class Input:
    ...
    def __init__(self, **options):
        """ Options: x, y, font, color, restricted, maxlength, prompt area"""
        self.options = Config(options, ['x', '0'], ['y', '0'], ['font', 'pygame.font.Font(None, 18)'],
                              ['color', '(0,0,0)'], ['restricted', '\'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!"#$%&\\\'()*+,-./:;<=>?@[\]^_`{|}~\''],
                              ['maxlength', '-1'], ['prompt', '\'\''], ['area', '(0,0,0,0)'])
        self.x = self.options.area[0]; self.y = self.options.area[1]
        self.font = self.options.font
        self.color = self.options.color
        self.restricted = self.options.restricted
        self.maxlength = self.options.maxlength
        self.prompt = ''; self.value = self.options.prompt
        self.shifted = False
        self.area = self.options.area
        
        
    def button_click(self, area, last):
        if area[0] < last[0] and area[1] < last[1] and area[0]+area[2] > last[0] and area[1]+area[3] > last[1]:
            return True
        return False

    def button_alloc(self, area, title, screen):
        label = self.font.render(title, 1, (0,0,0))
        screen.blit(label, (area[0], area[1]))
        pygame.draw.rect (screen, (0,0,0), Rect(area[0]-5, area[1]-5, area[2], area[3]), 1)

        if pygame.mouse.get_pressed()[0]:
            if self.button_click(area, pygame.mouse.get_pos()):
                return True
        return False

    def input_alloc(self, screen, active, events, c_active):
        if self.button_alloc(self.area,"", screen):
            c_active = active
        
        if c_active == active: 
            self.set_pos(self.area[0],self.area[1])
            self.update(events)
            self.draw(screen,"|")
    ...
    def draw(self, surface, active):
        ...
        text = self.font.render(self.prompt+self.value+active, 1, self.color)
